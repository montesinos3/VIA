...
for k, (key,frame) in enumerate(autoStream()):
    ...
    if results.multi_face_landmarks:
        ...
        for face_landmarks in results.multi_face_landmarks:
            ...
            selected = [(int(lan.x*w), int(lan.y*h)) for k in [33, 133, 362, 263] for lan in [face_landmarks.landmark[k]] ]

            x0,y0 = np.min(selected,axis=0)
            x1,y1 = np.max(selected,axis=0)

            # Hacer el rectángulo más alto
            x0 = max(0, x0 - 10)  # Mover el borde izquierdo
            x1 = min(w, x1 + 10)
            y0 = max(0, y0 - 10)  # Elevar el borde superior
            y1 = min(h, y1 + 10)  # Bajar el borde inferior
